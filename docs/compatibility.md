...
| controller-runtime | 0.25.0 | 0.22 - 0.25 | v0.25.0's breaking changes are the new EventRecorder interface and a bump to k8s.io/* v1.37, neither in kure's surface: kure imports only pkg/client (verified — no other controller-runtime subpackage is imported anywhere), whose Client interface is unchanged; IndexerFunc/FieldIndexer became type aliases to a new cacheapi package but kure uses neither. kure already requires k8s.io/api v0.37.0 directly, so the k8s.io/* bump is a no-op here. Build and tests pass unchanged. |
