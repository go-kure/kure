...
|------|---------------|-------------------------|-------|
| barman-cloud | 0.5.2-0.20260720143032-950b0f57e122 | 0.5 | Barman Cloud object-store API types (BarmanObjectStoreConfiguration, S3Credentials) — a separate module from both the CNPG operator and the barman-cloud *plugin* (cnpg-barman-cloud-plugin, below). Kure imports pkg/api directly. Pinned to the pseudo-version plugin-barman-cloud v0.14.0 itself requires, i.e. the minimum-version-selection floor, not an independently chosen commit. It moves only when the plugin moves; renovate.json disables the standalone gomod bump for it (see the rule there). |
