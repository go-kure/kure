...
| external-secrets | 0.0.0-20260824214743-7460a7a34f8d | 1.3 | Module path changed from root to /apis submodule in v1.0 (#5494). No semver tags for apis submodule — go.mod pins a pseudo-version to a release commit, so the range guard skips it (pseudo-versions can't map to semver). v1.3.2+, at the pseudo-version pinned in go.mod (includes controller-runtime v0.23.1 compat). |
