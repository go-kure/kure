...
> [Builder contract: release 1 migration notes](https://www.gokure.dev/kure/dev/concepts/builder-contract-release-1/):
...
> [Kubernetes Builders](https://www.gokure.dev/kure/dev/api-reference/kubernetes-builders/).

> **Builder contract, release 2.** The `Kind(&Config)` layer — one function per kind taking a
> kure-invented `Config` struct — is retired in `certmanager`, `cnpg`, `externalsecrets`,
> `prometheus`, `cilium` and `volsync`, together with the sealed `IssuerVariant`, `ACMESolver`,
