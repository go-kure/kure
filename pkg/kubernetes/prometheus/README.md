...
The config-struct builders (`prometheus.ServiceMonitor(&prometheus.ServiceMonitorConfig{...})`) are a separate, opinionated layer on top of the same upstream types. They now delegate to the generated constructors and start from identity only, and they fill the list fields inside a loop, so a config that supplies none leaves the field nil where it used to be an empty slice: `ServiceMonitor(cfg)` with no endpoints for `spec.endpoints`, `PodMonitor(cfg)` with none for `spec.podMetricsEndpoints`. Upstream declares both without `omitempty`, so what was `[]` is now `null`. `PrometheusRule(cfg)` with no groups drops the same default, but `spec.groups` does carry `omitempty`, so its emitted YAML is unchanged. All three removals are recorded in the release-1 migration ledger. One hand-written constructor for a spec fragment remains, `CreateRuleGroup`: `monitoringv1.RuleGroup` is not a `client.Object`, so it gets no generated wrapper, and the helper stays because a rule group is the unit a caller assembles repeatedly. Every other sub-type takes a struct literal.
