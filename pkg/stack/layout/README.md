...
Without a `ClusterName` the root node sits at `{rootName}` and every child node nests under it (`{rootName}/{childName}/...`), the paths the Flux generator derives from the node hierarchy. An unnamed root node has no directory of its own and stays at `cluster` (children at `cluster/{childName}`), as before. `WalkClusterByPackage` places each package's root the same way, below the package directory. A node outside the package adds no directory of its own: its in-package descendants attach to the nearest in-package ancestor (or the package root), including where the tree leaves a package and re-enters it lower down.
